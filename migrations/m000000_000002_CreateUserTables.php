...
class m000000_000002_CreateUserTables extends \yii\db\Migration
{
  ...
  public function safeUp()
  {
    ...
    $this->createTable(
      ...
      array(
        ...
        'status' => Schema::TYPE_INTEGER
          . ' NOT NULL DEFAULT '
          . \auth\models\User::STATUS_ACTIVE,
    ...
    $this->insert(
      $tableMap['ProfileFieldType'],
      array( 'name' => 'integer', 'title' => 'Integer' )
    ); //1
    $this->insert(
      $tableMap['ProfileFieldType'],
    ...
    $this->insert(
      $tableMap['ProfileFieldType'],
      array( 'name' => 'boolean', 'title' => 'Boolean' )
    ); //4
    $this->insert(
      $tableMap['ProfileFieldType'],
      array( 'name' => 'decimal', 'title' => 'Decimal' )
    ); //5
    $this->insert(
      $tableMap['ProfileFieldType'],
    ...
    $this->insert(
      $tableMap['ProfileFieldType'],
      array( 'name' => 'date', 'title' => 'Date only' )
    ); //7
    $this->insert(
      $tableMap['ProfileFieldType'],
      array( 'name' => 'datetime', 'title' => 'Date and Time' )
    ); //8
    $this->insert(
      $tableMap['ProfileFieldType'],
      array( 'name' => 'time', 'title' => 'Time only' )
    ); //9
    $this->insert(
      $tableMap['ProfileFieldType'],
      array( 'name' => 'url', 'title' => 'Url Address' )
    ); //10
    $this->insert(
      $tableMap['ProfileFieldType'],
    ...
    $this->insert(
      $tableMap['ProfileFieldType'],
      array( 'name' => 'lookup', 'title' => 'Lookup' )
    ); //12
    $this->insert(
      $tableMap['ProfileFieldType'],
    ...
    $this->createIndex(
      'ProfileField_name_uk',
      $tableMap['ProfileField'],
      'name',
      true
    );
    $this->createIndex(
      'ProfileField_type_ix',
      $tableMap['ProfileField'],
      'type_id'
    );
    $this->addForeignKey(
      'ProfileField_type_fk',
      $tableMap['ProfileField'],
      'type_id',
      $tableMap['ProfileFieldType'],
      'id'
    ...
    // $this->createIndex(
    //   'Profile_value_ix', $tableMap['ProfileFieldValue'], 'value'
    // );
    $this->addForeignKey(
      'Profile_user_fk',
      $tableMap['ProfileFieldValue'],
    ...
    $this->readStdinUser(
      'Email (e.g. admin@mydomain.com)', $adminUser, 'email'
    );
    $this->readStdinUser(
      'Type Username', $adminUser, 'username', $adminUser->email
    );
