 ...
 * the first match that will dictate whether to allow or deny the access to
 * the requested controller action. If no matches, the access will be denied.
 *
 * To use Auth AccessControl, declare it in the `behaviors()` method of
 * your controller class.
 * For example, the following declaration
 * will enable rbac filtering in your controller.
 *
 * ~~~
...
{
  /**
   * @var array name-value pairs that would be passed to business rules
   * associated with the tasks and roles assigned to the user.
   */
  public $params = [];

  /**
   * @var callable a callback that will be called if the access should be
   * denied to the current user. If not set, [[denyAccess()]] will be called.
   *
   ...
   * ~~~
   *
   * where `$item` is this item name, and `$action` is
   * the current [[Action|action]] object.
   */
  public $denyCallback;
  ...
  public function beforeAction( $action )
  {
    ...
    if ( $controller->module !== null ) {
      if ( $user->checkAccess(
        $this->getItemName( $controller->module ) . $this->separator . '*',
        $this->params
      ) ) {
        return true;
      }
    ...
    if ( $user->checkAccess(
      $this->getItemName( $controller ) . $this->separator . '*', $this->params
    ) ) {
      return true;
    }
   ...
   * The default implementation will redirect the user to the login page
   * if he is a guest;
